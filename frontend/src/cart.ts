...
const fetchGalleryItem = (id: string): Promise<GalleryItem> => {
    ...
    fetch("http://localhost:3000/cart", {
      ...
      .then((cartData: CartItem[]) => {
          ...
          const fetchPromises = cartData.map(cartItem => {
              return fetchGalleryItem(cartItem.id).then((item: GalleryItem) => {
...
const updateCartItemQuantity = (id: string, quantity: number): void => {
